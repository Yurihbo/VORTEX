  ...
  return (
    <Layout>
      <div className="space-y-9 animate-fade-in">
        ...
        {bookOfDay && <section><div className="mb-5"><p className="eyebrow">Uma sugestão para hoje</p><h2 className="mt-1 text-4xl font-serif">✦ Grimório do Dia</h2></div><Card className="vortex-card p-6 md:p-8"><div className="grid gap-7 md:grid-cols-[140px_1fr] items-center"><BookCover book={bookOfDay} size="lg" className="mx-auto md:mx-0" /><div><div className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[.18em] text-[#caa85e]"><span>{bookOfDay.genre}</span><span className="text-muted-foreground">·</span><span>{bookOfDay.pages} páginas</span></div><h3 className="mt-3 text-4xl font-serif">{bookOfDay.title}</h3><p className="mt-1 text-muted-foreground">{bookOfDay.author}</p><p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">{bookOfDay.description}</p><Link href={`/book/${bookOfDay.id}`} className="mt-6 inline-flex"><Button>Explorar livro</Button></Link></div></div></Card></section>}
      </div>
    </Layout>
  );
}
