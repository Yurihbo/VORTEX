    ...
    <Layout>
      <div className="space-y-7 animate-fade-in">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          ...
          <Card className="vortex-card p-7"><div className="flex items-start justify-between"><div><p className="eyebrow">Constância</p><h2 className="text-4xl font-serif mt-1">Chama da Leitura</h2></div><Flame className="h-7 w-7 text-[#caa85e]" /></div><div className="mt-8 flex items-baseline gap-2"><span className="text-6xl font-serif text-[#caa85e]">{streak.currentStreak}</span><span className="text-muted-foreground">dias</span></div><p className="mt-2 text-muted-foreground">Você mantém a chama acesa há {streak.currentStreak} dias.</p><div className="mt-7 grid grid-cols-2 gap-3"><div className="goal-streak-stat"><p className="goal-stat-label">Melhor sequência</p><p className="goal-stat-value">{streak.bestStreak} dias</p></div><div className="goal-streak-stat"><p className="goal-stat-label">Próximo marco</p><p className="goal-stat-value">{Math.max(0, 7 - (streak.currentStreak % 7))} dias</p></div></div><Button variant="outline" className="mt-6 w-full" onClick={updateStreak}><Flame className="h-4 w-4 mr-2" /> Registrar leitura de hoje</Button></Card>
        </div>
